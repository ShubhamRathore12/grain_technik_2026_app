...
const groupInputsByType = (inputs: Array<{ description: string; field: string }>) => {
    const grouped = {
...
const getAnalogConfig = (device: string, data: any) => {
    ...
    const grouped = groupInputsByType(analogInputs);
    
    // Create flat array of all inputs for simpler rendering
    const allInputs = [
...
const getIcon = (description: string) => {
    if (description.toLowerCase().includes('pressure')) return <Gauge size={16} color="#3b82f6" />;
    if (description.toLowerCase().includes('temp')) return <Thermometer size={16} color="#f97316" />;
...
export default function AnalogInputsScreen() {
    ...
    const renderItem = ({ item }: { item: AnalogItem }) => {
        ...
        let progress = 0;
        if (item.isPressure) {
            progress = Math.min(100, (numValue / 300) * 100); // Assuming max pressure of 300 psi
        } else if (item.isRTD) {
